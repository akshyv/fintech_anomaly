...
function loadAuditLog() {
    ...
    // Show loading state
    container.innerHTML = '<div style="text-align: center; padding: 20px;"><div class="spinner"></div><p style="margin-top: 10px; color: #718096;">Loading audit log...</p></div>';
    
    fetch(`${API_URL}/audit-log?limit=10`)
        .then(response => {
            if (!response.ok) {
        ...
        .then(data => {
            ...
            if (!data.logs || data.logs.length === 0) {
                container.innerHTML = '<p style="color: #888; padding: 20px; text-align: center;">No audit entries yet. Generate and score some transactions first!</p>';
                return;
            }
            ...
            let html = `
                <div style="font-size: 12px; color: #888; margin-bottom: 10px;">
                    📊 Showing ${data.count} most recent decision(s)
                </div>
            ...
            data.logs.forEach(log => {
                ...
                // Color code decision
                let decisionClass = '';
                if (log.decision === 'APPROVE') decisionClass = 'decision-approve';
                else if (log.decision === 'DECLINE') decisionClass = 'decision-decline';
                else decisionClass = 'decision-review';
                
                // Color code risk score
                let riskColor = '';
                if (log.risk_score > 0.7) riskColor = '#dc3545';
                else if (log.risk_score > 0.4) riskColor = '#ffc107';
                else riskColor = '#28a745';
                
                // Truncate explanation if too long
                const explanation = log.explanation 
                    ? (log.explanation.length > 80 
                        ? log.explanation.substring(0, 80) + '...' 
                        : log.explanation)
                ...
                html += `
                    <tr>
                        ...
                        <td style="text-align: center;">#${log.transaction_id}</td>
                        <td style="color: ${riskColor}; font-weight: bold; text-align: center;">${(log.risk_score * 100).toFixed(1)}%</td>
                        <td><span class="${decisionClass}">${log.decision}</span></td>
                        <td style="font-size: 12px; max-width: 300px;">${explanation}</td>
                    </tr>
                `;
            });
        ...
        .catch(error => {
            ...
            container.innerHTML = `
                <div style="background: #fff5f5; border-left: 4px solid #e53e3e; padding: 15px; border-radius: 4px;">
                    <p style="color: #e53e3e; margin: 0; font-weight: 600;">
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Auto-load audit log after short delay
    setTimeout(() => {
        loadAuditLog();
    }, 500);
});