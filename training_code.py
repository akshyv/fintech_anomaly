    ...
    def load_or_train_model(self):
        ...
        if os.path.exists(model_path):
            ...
            normal_data = np.column_stack([
                np.random.uniform(0.5, 1.5, n_samples), #amount_ratio
                np.random.randint(8, 20, n_samples), #hour
                np.random.randint(0, 7, n_samples), #day_of_week
                np.random.uniform(0.1, 0.4, n_samples), #category_risk
                np.random.uniform(100, 1000, n_samples) #account_age_days
            ])
